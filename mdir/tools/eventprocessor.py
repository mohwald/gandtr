...
# - blob (image - rgb, rgba, gray; binary - response, vector, text)
...
class MetadataKeeper(EventSink):
    ...
    def register_epoch_data(self, epoch, data, consts):
        ...
        for key, item in data.items():
            if key in self.keys:
                assert self.keys[key] == item["data"].keys(), (self.keys[key], item['data'].keys())
                continue
...
class EpochEventAccumulator(EventStreamer):
    ...
    folder_name = "blobs"
    histogram_bins = 200
    dtypes = {"scalar/loss", "scalar/score", "scalar/time", "weight/param", "weight/grad", "blob", "heatmap"}
    suffixes = {
        "image:rgb": "png",
        "image:rgba": "png",
        "image:gray": "png",
    ...
    def _store_blob(self, fname_piece, key, data):
        ...
        for subkey, value in data.items():
            ...
            self._store_single_blob(value.pop("data"), self.datapath / fname, suffix)
            value["path"] = os.path.join(self.folder_name, fname)
        return data

    def _store_heatmap(self, fname_piece, key, data):
        data = self._make_datapath(data)
        if self.datapath is None:
            return data

        for subkey, value in data.items():
            fname = "%s:%s:%s.%s" % (key.replace("/", "_"), subkey.replace("/", "_"), fname_piece, "png")
            plt.figure(figsize=(6, 6))
            plt.axis('off')
            if "loss" in key or "loss" in subkey:
                plt.imshow(value.pop("data"), vmin=0.0, vmax=1.0, cmap='Reds', interpolation='nearest')
            else:
                plt.imshow(value.pop("data"), vmin=0.0, vmax=1.0, cmap='coolwarm', interpolation='nearest')
            plt.savefig(self.datapath / fname, transparent=True, bbox_inches='tight')
            plt.close()
            value["path"] = os.path.join(self.folder_name, fname)
        return data
    ...
    def add_row(self, epoch, timestamp, relative_iteration, epoch_size, key, data, dtype):
        ...
        if dtype == "blob":
            ...
            rel_iter = "null" if relative_iteration is None else relative_iteration
            data = self._store_heatmap("%s:%s" % (epoch, rel_iter), key, data)
        ...
        for row in self.accumulator:
            if row["relative_iteration"] is None:
                ...
                assert aggregated[row["key"]]["data"].keys() == row["data"].keys(), \
                    "%s: %s != %s" % (row["key"], aggregated[row["key"]]["data"].keys(), row["data"].keys())
        ...
        if key.endswith(self.key_suffix[0]) and self.key_suffix[1] in data:
            ...
            if relative_iteration1 is None or relative_iteration1 % print_each == 0 \
                ...
                sys.stderr.write("%s [%0.2d][%0.3d/%0.3d]: %.2f (%.2f), (%.2f s/b%s)\n" % \
                        ...
                        (timestamp - history["first_timestamp"]) / max(history["n"] - 1, 1), additional_timing))
...
class HtmlReport(EventSink):
        ...
        for key, item in data.items():
            *key, key2 = key.split("/", 2)
            ...
            if key not in self.data:
                self.data[key] = {}
            self.__store_multiple_loss(epoch, key, key2, item)
            for subkey, subitem in item["data"].items():
                ...
                elif item["dtype"] == "blob" or item["dtype"] == "heatmap":
                    self.data[key][subkey]["plot_type"] = "thumbnail_set"
                    for i, subitem_item in enumerate(subitem):
                        path = os.path.join(self.dataroot, subitem_item["path"])
                        self.data[key][subkey]["data"].append({**subitem_item, "epoch": epoch, "iteration": item["relative_iteration"][i],
                                                               "thumbnail": self._square_thumbnail(path, 200)})

        for key, item in consts.items():
            *key, key2 = key.split("/", 2)
            key = "/".join(key)
            if key not in self.data and (item["dtype"] == "blob" or item["dtype"] == "heatmap"):
                self.data[key] = {}
                for subkey, subitem in item["data"].items():
                    subkey = "%s/%s" % (key2, subkey)
                    path = os.path.join(self.dataroot, subitem["path"])
                    self.data[key][subkey] = {**subitem, "plot_type": "thumbnail",
                                              "thumbnail": self._square_thumbnail(path, 200)}

    def __store_multiple_loss(self, epoch, key, key2, item):
        if len(item["data"]) > 1 and item["dtype"] == "scalar/loss":
            key2 += "/all"
            if key2 not in self.data[key]:
                self.data[key][key2] = {"plot_type": "multiplot", "subtype": "loss", "multi_data": {}}
            for subkey, subitem in item["data"].items():
                if subkey not in self.data[key][key2]["multi_data"]:
                    self.data[key][key2]["multi_data"][subkey] = {"epoch": [], "data": []}
                self.data[key][key2]["multi_data"][subkey]['epoch'].append(epoch + 1)
                self.data[key][key2]["multi_data"][subkey]['data'].append(np.mean(subitem))
    ...
    def _square_thumbnail(self, path, size):
        if path.endswith(".txt"):
            with open(path, 'r') as handle:
                pieces = [x.strip() for x in handle.read().split(",")]
                # Vertical elipsis
                if len(pieces) > 7:
                    pieces = pieces[:4] + ["..."] + pieces[-2:]
                # Horizontal elipsis
                for i, piece in enumerate(pieces):
                    if len(piece) > 11:
                        pieces[i] = pieces[i][:5] + "..." + pieces[i][-3:]
    ...
    def render(self, epoch):
        ...
        order = {"train/learning": 0, "val/learning": 1, "train/net": 3, "net": 4, "train/data": 5}
        sets = {}
        for key, item in sorted(self.data.items(), key=lambda x: order.get(x[0], 2 if "learning" in x[0] else 100)):
            section = []
            for subkey, subitem in item.items():
                ...
                if subitem["plot_type"] == "curve":
                ...
                elif subitem["plot_type"] == "multiplot":
                    fname %= "multiplot"
                    self.store_plot(fname, subitem["multi_data"], subitem["subtype"])
                    ...
                    fname = os.path.join("..", subitem["path"])
                    ...
                    for singleimg in subitem["data"]:
                        ...
                        fname = os.path.join("..", singleimg["path"])
                        if fname.endswith(".txt"):
                            data = {"type": "div", "data": singleimg["thumbnail"], "link": fname, "css": "margin: 0 10px 0 10px; min-width: 100px"}
                        else:
                            data = {"type": "image", "source": singleimg["thumbnail"], "link": fname, "size": 200}
                        sets[key][name].append({"type": "blocks", "name": subkey.replace("/", "<br />"), "data": [data]})
                    continue
                else:
                    continue
