...
def init_network(params):
    ...
    if pretrained:
        if isinstance(pretrained, str) and pretrained.startswith("torch.hub://"):
            # initialize network from hub
            net_in = torch.hub.load(pretrained[len("torch.hub://"):], architecture) # pretrained=True kwarg raises an exception
        elif isinstance(pretrained, str) and pretrained.startswith("python.lib://"):
            # initialize network from external library
            path, params = pretrained[len("python.lib://"):].split("/")
            *module, func = path.split(".")
            module = functools.reduce(lambda mod, piece: getattr(mod, piece), module[1:], __import__(".".join(module)))
            parse_params = lambda x, y: (x, {'True': True, 'False': False}.get(y, y))
            params = dict(parse_params(*x.split("=")) for x in params.replace("[architecture]", architecture).split(","))
            net_in = getattr(module, func)(**params)
        elif architecture not in FEATURES and not isinstance(pretrained, str):
    ...
    if architecture.startswith('alexnet'):
        ...
        features = list(net_in.features.children())[:-1]
    elif architecture.startswith('resnet') or architecture.startswith('resnext') or architecture.startswith("regnet"):
    ...
    last_layers = list(features[-2].modules()) + list(features[-1].modules())
    last_convs = [x for x in last_layers if isinstance(x, nn.Conv2d)]
    dim = last_convs[-1].out_channels
    if architecture in OUTPUT_DIM and dim != OUTPUT_DIM[architecture]:
        warnings.warn(f"Dimensionality mismatch: overwriting preset {OUTPUT_DIM[architecture]} with network's {dim}", RuntimeWarning)
    ...
    elif not isinstance(pooling, dict):
    ...
    if isinstance(pretrained, str) and (pretrained.startswith("torch.hub://") or pretrained.startswith("python.lib://")):
        pass
    elif pretrained and isinstance(pretrained, str):
        print(">> {}: for '{}' custom pretrained features '{}' are used"
            .format(os.path.basename(__file__), architecture, os.path.basename(pretrained)))
        net.features.load_state_dict(torch.load(pretrained))
    elif pretrained and architecture in FEATURES:
        print(">> {}: for '{}' custom pretrained features '{}' are used"
