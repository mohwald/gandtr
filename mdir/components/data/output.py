...
class EmbeddingOutput(DataOutput):
    ...
    def __init__(self, data, _data_params, *, use_bbxs=False, image_labels=False):
        """ Input:
                data -- tuple containing columns, always a list of image paths,
                        optionally bounding boxes and/or image labels.

            Outputs (image paths, np array (len(images,), dim))
        """
        self.image_labels = None
        if image_labels:
            *data, self.image_labels = data
        if not use_bbxs:
            assert len(data) == 1, len(data)
        self.images, self.bbxs = data if use_bbxs else (data[0], None)
        self.vecs = None
